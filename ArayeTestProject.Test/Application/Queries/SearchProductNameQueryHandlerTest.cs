    ...
    public class SearchProductNameQueryHandlerTest
    {
        ...
        public async Task Handle_Test_With_Data()
        {
            try
            {
                ...
                repositoryMock.Setup(rm => rm.GetProductNames(It.IsAny<string>())).ReturnsAsync(GetTestNames());

                SearchProductNameQueryHandler handler = new SearchProductNameQueryHandler(repositoryMock.Object);
        ...
        public async Task Handle_Test_With_No_Data()
        {
            ...
            {
                var repositoryMock = new Mock<IMainRepository>();
                repositoryMock.Setup(rm => rm.GetProductNames(It.IsAny<string>())).ReturnsAsync(new List<string>());
